...
fs.writeFileSync('/tmp/host.html',
...
(async () => {
  ...
  ok('the banner explains what this is', /leaves your computer/.test(await f.locator('.trybar').innerText()));
  ...
  ok('asks for something before copying nothing',
    /at least one box/i.test(await f.locator('#fbNote').innerText()));
  ...
  ok('it records what they chose', /Marlow and Finch|A website/.test(out));
  ok('it records what they wrote', /not obvious/.test(out) && /450 feels high/.test(out));
  ok('it skips the boxes they left blank', !/WHAT'S MISSING/.test(out));
  ok('it tells them what happened either way',
    /copied|select the text/i.test(await f.locator('#fbNote').innerText()),
    await f.locator('#fbNote').innerText());

  await p.screenshot({ path: __dirname + '/shareable.png', fullPage: false });
