...
| `RENTSTORE_DOMAIN` | `rentstore.ae` | Builds each seller's page URL |
| `RENTSTORE_PRICE` | `55` | Monthly price, in the messages |
| `RENTSTORE_GRACE_DAYS` | `7` | Days late before a store switches off |
| `RENTSTORE_SETUP_FEE` | `1500` | Your one-time setup fee, in the messages |
...
```js
window.PLATFORM = {
  signupWebhook: "https://<your-n8n>/webhook/rentstore-signup",
  setupWebhook:  "https://<your-n8n>/webhook/rentstore-setup-request",
  …
}
window.SITE = { orderWebhook: "https://<your-n8n>/webhook/rentstore-order", … }
```
...
`rentstore-stripe` URL, sending `checkout.session.completed` and `invoice.paid`.
...
4. **Setup** — POST a name and phone to `rentstore-setup-request`; a case id comes back and both
   messages go out. Then POST `{"case_id":"…","phone":"…","stage":"filed"}` to
   `rentstore-setup-stage` and check the customer gets the matching update.
