...
readable page from drifting apart.
    ...
    python3 projects/rentstore/automation/maps/build-maps.py
"""
...
MAPS = [
 ("map-01-merchant-journey.json", "MAP · The merchant's journey", "merchant",
   ...
   (1, "They join, free",
    ...
    [("input", "👤 They find us", [
     ...
     ("input", "✍️ They register", [
        ...
        ("M5 · Their page exists", "rentstore.ae/@handle — links, socials, WhatsApp button. Usable today."),
  ...
  [("📌 The numbers on this canvas",
    ...
    "Change them in `content/data.js` and `RENTSTORE_PRICE`, not here.", 320),
...
def build():
    ...
    for fn, title, lane, sub, trig, steps, panels in MAPS:
        ...
        c.link(*order)
        lines.append(c.save(HERE / fn, tag="rentstore-map"))
