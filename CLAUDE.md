...
- `site/` — static bilingual (AR/EN, RTL) multi-vendor marketplace: home, store directory (`vendors.html`), per-seller storefront (`vendor.html?id=`), all-products grid, and product page. Each product belongs to a vendor; a cart spanning several stores splits into one order per seller. `apply.html` is a self-serve store builder with a live preview that emits a JSON submission for the owner to paste in. No build step; all content lives in `site/assets/js/data.js` (`SITE`, `VENDORS`, `PRODUCTS`). See `site/README.md`.
